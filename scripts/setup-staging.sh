...
STAGING_REF="geghodbcxmaqtvyhwhjm"
VERCEL_TOKEN="REDACTED_VERCEL_TOKEN"
PROJECT_ID="prj_bEyF8QFVm8nQYIPp0pus4YnCoVIA"
...
curl -s -X PATCH \
  ...
  -d '{
    ...
    "external_github_client_id": "REDACTED_GITHUB_CLIENT_ID",
    "external_github_secret": "REDACTED_GITHUB_OAUTH_SECRET",
    "external_google_enabled": true,
    "external_google_client_id": "REDACTED_GOOGLE_CLIENT_ID",
    "external_google_secret": "REDACTED_GOOGLE_OAUTH_SECRET",
...
curl -s -H "Authorization: Bearer ${VERCEL_TOKEN}" \
  "https://api.vercel.com/v9/projects/${PROJECT_ID}/env" | python3 -c "
import sys, json
...
if [ -n "$CURRENT_IP" ]; then
  curl -s -X PATCH -H "Authorization: Bearer ${VERCEL_TOKEN}" -H "Content-Type: application/json" \
    "https://api.vercel.com/v9/projects/${PROJECT_ID}/env/wGZbfbPjYxJzaQOI" \
...
echo "  https://geghodbcxmaqtvyhwhjm.supabase.co/auth/v1/callback"
