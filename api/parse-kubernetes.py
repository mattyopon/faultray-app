"""Kubernetes manifest parsing endpoint.

POST /api/parse-kubernetes
  Body: { "manifests": "<yaml/json string>", "namespace"?: string }

Parses Kubernetes manifests and runs FaultRay simulation.
"""
...
def _run_k8s_parse(manifests: str, namespace: str | None = None) -> dict:
    ...
    # K8sScanner uses kubeconfig; we write the manifests to a temp kubeconfig
    # and set KUBECONFIG env var. However, if manifests is kubectl output
    # (JSON/YAML with resource definitions), we parse them directly.
    # For direct manifest parsing, we use the scanner's internal methods
    # by setting up a minimal kubeconfig pointing to the manifests.
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            self._send_error(
                500, f"Kubernetes parse error ({error_type}): {e}"
            )
