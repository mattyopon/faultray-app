"""FaultRay health check + admin check + admin plan switch endpoint.

GET /api/health -> { status, engine, version }
POST /api/health -> depends on action:
  { "action": "admin-check", "email": "..." } -> { is_admin }
...
def _switch_plan(email: str, plan: str) -> dict:
    """Update plan in Supabase using service_role key (bypasses RLS)."""
    ...
    def do_POST(self):
        ...
        if action == "admin-check":
            ...
            if not _is_admin(email):
                ...
                return
            plan = body.get("plan", "")
            result = _switch_plan(email, plan)
            if "error" in result:
