    ...
    <Card className="cursor-pointer" onClick={() => setExpanded(!expanded)}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <Badge variant="default">{(layer.weight * 100).toFixed(0)}% weight</Badge>
        </div>
      ...
      <p className="text-xs text-[#64748b]">
        Weighted contribution: {layer.weighted_score.toFixed(2)} points
      </p>
      ...
      <p className="text-xs text-[#64748b] mt-2">{expanded ? "Click to collapse" : "Click to expand factors"}</p>
    </Card>
  );
}
    ...
    <div className="max-w-[1200px] mx-auto px-6 py-10">
      <div className="mb-10">
        ...
        <p className="text-[#94a3b8] text-sm">
          Understand what drives your resilience score
        </p>
      </div>
      ...
      {loading ? (
        <Card className="flex items-center justify-center py-20">
          ...
          <span className="ml-3 text-[#94a3b8]">Loading score breakdown...</span>
        </Card>
      ) : (
        <>
          {/* Overall Score */}
          <Card className="mb-6 text-center">
            <p className="text-xs text-[#64748b] uppercase tracking-wider mb-2">Overall Resilience Score</p>
            <p className="text-6xl font-extrabold font-mono" style={{ color: scoreColor(data.overall_score) }}>
              {data.overall_score.toFixed(1)}
            </p>
            <p className="text-sm text-[#94a3b8] mt-2">
              Sum of weighted layer scores
            </p>
          </Card>
                  ...
                  {data.top_detractors.map((d) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant="green">+{d.potential_gain.toFixed(1)} pts</Badge>
                      </div>
                    </div>
                  ))}
                </div>
