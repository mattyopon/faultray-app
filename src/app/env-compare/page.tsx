    ...
    <div className="w-full px-6 py-10">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-1 flex items-center gap-3">
          <GitCompare size={24} className="text-[var(--gold)]" />
          {t.title}
        </h1>
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        <Card className="text-center">
          <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-2">{envA.label}</p>
          <p className={`text-4xl font-extrabold font-mono ${envA.score >= 85 ? "text-[#10b981]" : "text-[#f59e0b]"}`}>
            {envA.score}
          </p>
          <p className="text-xs text-[var(--text-muted)] mt-1">{locale === "ja" ? "レジリエンススコア" : "Resilience Score"}</p>
        </Card>
        <Card className="text-center flex flex-col items-center justify-center">
          <ArrowRight size={24} className="text-[var(--text-muted)]" />
          <p className="text-xs text-[var(--text-muted)] mt-2">{t.differences}: <span className="text-[#f59e0b] font-bold">{diffKeys.length}</span></p>
        </Card>
        <Card className="text-center">
          <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-2">{envB.label}</p>
      ...
      <Card className="mb-6">
        <p className="text-sm font-semibold text-[var(--gold)] mb-4">{t.component} {locale === "ja" ? "比較" : "Comparison"}</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {envA.components.map((comp, i) => {
                ...
                return (
                  <tr key={comp.name} className="border-b border-[var(--border-color)]/50 hover:bg-white/2">
                    <td className="py-2.5 pr-4 font-semibold">{comp.name}</td>
                    <td className="py-2.5 pr-4 text-center"><ScorePill score={comp.score} /></td>
                    <td className="py-2.5 pr-4 text-center font-mono text-xs text-[var(--text-secondary)]">{comp.version}</td>
                    <td className="py-2.5 pr-4 text-center"><ScorePill score={compB.score} /></td>
                    <td className="py-2.5 pr-4 text-center font-mono text-xs text-[var(--text-secondary)]">
                      <span className={versionSame ? "" : "text-[#f59e0b]"}>{compB.version}</span>
                    </td>
                    <td className="py-2.5">
                      <span className={`font-mono text-sm font-bold ${delta > 0 ? "text-[#10b981]" : delta < 0 ? "text-red-400" : "text-[var(--text-muted)]"}`}>
                        {delta > 0 ? `+${delta}` : delta}
                      </span>
                    </td>
                  </tr>
      ...
      {/* Config diff */}
      <Card>
        <p className="text-sm font-semibold text-[var(--gold)] mb-4">{t.configDiff}</p>

        {/* Different keys */}
        {diffKeys.length > 0 && (
          <div className="mb-4">
            <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-2">{t.different} ({diffKeys.length})</p>
            <div className="space-y-2">
              {diffKeys.map((key) => (
                <div key={key} className="grid grid-cols-3 gap-2 rounded-lg p-3">
                  <div>
                    <p className="text-xs text-[var(--text-muted)] mb-1 font-mono">{key}</p>
                  </div>
                  <div className="bg-[var(--border-color)]/50 rounded p-1.5">
                    <p className="text-xs text-[var(--text-muted)] mb-0.5">{envA.label}</p>
                    <code className="text-xs text-[var(--text-secondary)]">{envA.config[key]}</code>
                  </div>
                  <div className="bg-[#f59e0b]/10 rounded p-1.5 border border-[#f59e0b]/20">
                    <p className="text-xs text-[var(--text-muted)] mb-0.5">{envB.label}</p>
        ...
        {/* Identical keys */}
        <div>
          <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider mb-2">{t.identical} ({sameKeys.length})</p>
          <div className="grid md:grid-cols-2 gap-2">
            {sameKeys.map((key) => (
              <div key={key} className="flex items-center justify-between rounded-lg px-3 py-2">
                <code className="text-xs text-[var(--text-muted)] font-mono">{key}</code>
